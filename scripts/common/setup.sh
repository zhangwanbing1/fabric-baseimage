...
# ----------------------------------------------------------------
mkdir -p $GOPATH
ARCH=`uname -m | sed 's|i686|386|' | sed 's|x86_64|amd64|' | sed 's|armv7l|armv6l|' | sed 's|aarch64|arm64|'`
BINTARGETS="x86_64 ppc64le s390x armv7l armv6l aarch64"
GO_VER=1.11.5
...
# Otherwise, build Golang from source
   ...
   cd /tmp
   wget --quiet --no-check-certificate https://studygolang.com/dl/golang/go${GO_VER}.src.tar.gz
